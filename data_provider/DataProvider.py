...
class DataProvider:
    ...
    # 下载股票历史日数据
    def download_stock_daily_data(self, stock_symbols, start_date, end_date, period, max_workers, callback):
        tasks = []
        for symbol in stock_symbols:
            tasks.append({
        ...
        util.run_parallel_tasks(tasks, max_workers, f'正在下载{period}历史数据...', __callback)
